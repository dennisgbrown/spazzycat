...
;===============================================================================
    ...
    ;
    ; TIA object usage for Herding Cats:
    ;   player0     = left player
    ;   player1     = cat
    ;   missile0    = box
    ;   missile1    = box
...
;===============================================================================
    ...
    ; 2020.05.25 - start with 2014.07.14 revision of Collect 
    ;   See https://atariage.com/forums/blogs/entry/11157-step-14-add-animation/
...
PosObject:
        sec            ; [ Set Carry ]
        sta WSYNC      ; Strobe TIA WSYNC (CPU halts until TIA wakes it back up)
DivideLoop             ; Divide by 15 by repeatedly subtracting 15
        sbc #15        ; 2  2 - [ Subtract with Carry ] the value 15; each time thru this loop takes 5 cycles, which is 
        bcs DivideLoop ; 2  4 - [ Branch on Carry Set ] loop until carry not set; the same amount of time it takes to draw 15 pixels
        eor #7         ; 2  6 - [ Bitwise XOR with Accumulator ] The EOR & ASL statements convert the remainder
        asl            ; 2  8 - [ Shift Left ] of position/15 to the value needed to fine tune
        asl            ; 2 10 - [ Shift Left ] the X position
        asl            ; 2 12 - [ Shift Left ] 
        asl            ; 2 14 - [ Shift Left ] 
        sta.wx HMP0,X  ; 5 19 - [ Store Accumulator into HMP0 offset by X ] store fine tuning of X
                       ; See https://atariage.com/forums/topic/198465-i-want-to-keep-learning-a2600-programming-but-im-stuck/
                       ; "This instructs DASM to produce a store to a 16bit address 
                       ; eventhough HMP0 is a zero-page (8-bit) address. More precisely, 
                       ; sta.wx HMP0,x is replaced with 
                       ; sta $0020,x (absolute,x addressing, 3 bytes, 5 cycles) 
                       ; instead of sta $20,x (zero page,x addressing, 2 bytes, 4 cycles).
                       ; Here in this case this is just a simple trick to delay the write 
                       ; to the RES registers (that follows in the next instruction) by 
                       ; one cycle. This is necessary to get the timing right to have 
                       ; objects appear at their correct positions." 
        sta RESP0,X    ; 4 23 - [ Store Accumulator ] into RESP0 offset by X; set coarse X position of object
...
InitSystem:
    ...
    ; seed the random number generator
        lda INTIM       ; [ Load Accumulator ] with unknown value from timer
        sta Rand8       ; [ Store Accumulator ] into Random (8-bit) memory location; use as seed
        eor #$FF        ; [ XOR with Accumulator ] with hex value FF; both seed values cannot be 0, so flip the bits 
        sta Rand16      ; [ Store Accumulator ] into Random (16-bit) memory location; just in case INTIM was 0

    ; position objects and display variation=1 and players=1
        jsr NewGame     ; [ Jump to Subroutine ] put objects in default position
        inx             ; [ Increment X ] x was 0, now 1
        stx Score       ; [ Store X ] to score memory location; display human readable game variation
        stx Score+1     ; [ Store X ] to score memory location + 1; display human readable player count
...
;===============================================================================
VerticalBlank:
        jsr Random          ; [ Jump to Subroutine ] Random to randomize Rand8 and Rand16
        jsr ProcessSwitches ; [ Jump to Subroutine ] ProcessSwitches
        bit GameState       ; [ test BIT ] of value in memory location GameState
        bpl NotActive       ; [ Branch if Plus ] if GameState D7 == 0, game is not active; jump to NotActive
                            ; skip timer and joystick if game is not active
        jsr UpdateTimer     ; [ Jump to Subroutine ] update the timer
        jsr ProcessJoystick ; [ Jump to Subroutine ] process the joystick inputs
NotActive:        
        jsr PositionObjects ; [ Jump to Subroutine ] position the 5 objects
        jsr SetObjectColors ; [ Jump to Subroutine ] set colors of the objects
        jsr PrepScoreForDisplay ; [ Jump to Subroutine ] prep score for display
        rts                 ; [ ReTurn from Subroutine ]
...
;===============================================================================
UpdateTimer:
        lda Frame       ; [ Load Accumulator ] from memory a location Frame
        and #63         ; [ AND Accumulator ] with value 63 (binary 0111111)
        beq TimerTick   ; [ Branch if Equal to zero ] to TimerTick; tick the timer once every 64 frames
        rts             ; [ Return from Subroutine ] otherwise return
        
TimerTick:
        lda TimerPF         ; [ Load Accumulator ] with value at memory location TimerPF
        and #%11110000      ; [ AND Accumulator ] with  binary value 11110000; and chop off the lower nybble
        bne DecrementTimer  ; [ Branch if Not Equal to zero ] to DecrementTimer; if the upper nybble is not 0 then time remains
        lda #0              ; [ Load Accumulator ] with value 0; otherwise time ran out so end the game
        sta GameState       ; [ Store Accumulator ] to memory location GameState
        ldy #sfxGAMEOVER    ; [ Load Y ] with value sfxGAMEOVER; Game Over sound effect
        jsr SFX_TRIGGER     ; [ Jump to Subroutine ] to trigger sound effect
        rts                 ; [ Return from Subroutine ] 
        
DecrementTimer:
        lsr TimerPF+5   ; [ Logical Shift Right ] PF2 right side, reversed bits so shift right
        rol TimerPF+4   ; [ Rotate Left ] PF1 right side, normal bits so shift left
        ror TimerPF+3   ; [ Rotate Right ] PF0 right side, reversed bits so shift right
                        ; Note: ROR shifts Carry to D7 while LSR does not
        lda TimerPF+3   ; [ Load Accumulator ] with value in TimerPF+3; only upper nybble used, so we need to put bit 3 into C
        lsr             ; [ Logical Shift Right ]
        lsr             ; [ Logical Shift Right ]
        lsr             ; [ Logical Shift Right ]
        lsr             ; [ Logical Shift Right ]
        ror TimerPF+2   ; [ Rotate Right ] PF2 left side, reversed bits so shift right
        rol TimerPF+1   ; [ Rotate Left ] PF1 left side, normal bits so shift left
        ror TimerPF     ; [ Rotate Right ] PF0 left side, reversed bits so shift right
        lda TimerPF+1   ; [ Load Accumulator ] with value in TimerPF+1; PF1 from left side
        and #%00011111  ; [ AND Accumulator ] check the lower 5 bits
        bne NoTickSfx   ; [ Branch if Not Equal to zero ] branch if there's a value in the lower 5 bits to skip sound effect
        ldy #sfxPING    ; [ Load Y ] with value sfxPING
        jsr SFX_TRIGGER ; [ Jump to Subroutine ] to play sound effect
NoTickSfx:        
        rts             ; [ Return from Subroutine ] 
...
;===============================================================================
ProcessJoystick:
        lda SWCHA       ; [ Load A ] reads joystick positions
        
        ldx #0          ; [ Load X ] with value 0; x=0 for left joystick, x=1 for right
PJloop:    
        ldy ObjectX,x   ; [ Load Y ] save original X location so the player can be
        sty SavedX,x    ; [ Store Y ]   bounced back upon colliding with the playfield
        ldy ObjectY,x   ; [ Load Y ] save original Y location so the player can be
        sty SavedY,x    ; [ Store Y ]  bounced back upon colliding with the playfield
        asl             ; [ Arithmetic Shift Left ] of accumulator; shift A bits left, R is now in the carry bit
        bcs CheckLeft   ; [ Branch on Carry Set ] branch if Carry set (R not pressed) -- 
                        ;    joystick is not held right so go check other possibilities
                        ; Joystick IS held right:
        ldy ObjectX,x   ; [ Load Y ] get the object's X position
        iny             ; [ Increment Y ] and move it right
        cpy #160        ; [ Compare Y ] to value 160; if Y = 160, flag Z will be set; test for edge of screen
        bne SaveX       ; [ Branch Not Equal ] if flag Z not set, jump to SaveX; save Y if we're not at the edge
        ldy #0          ; [ Load Y ] with value 0 to wrap to left edge
SaveX:  sty ObjectX,x   ; [ Store Y ] save X position
        ldy #0          ; [ Load Y ] with value 0; turn off reflect of player, which
        sty REFP0,x     ; [ Store Y ] into REFP0; makes humanoid image face right

CheckLeft:
        asl             ; [ Arithmetic Shift Left ] shift A bits left; L is now in the carry bit
        bcs CheckDown   ; [ Branch if Carry Set ] branch if joystick not held left (Carry = 1)
        ldy ObjectX,x   ; [ Load Y ] get the object's X position
        dey             ; [ Decrement Y ] and move it left
        cpy #255        ; [ Compare Y ] to value 255; test for edge of screen
        bne SaveX2      ; [ Branch if Not Equal ] save X if we're not at the edge
        ldy #159        ; [ Load Y ] with value 159 to wrap to right edge
SaveX2: sty ObjectX,x   ; [ Store Y ] save X position
        ldy #8          ; [ Load Y] with value 8; turn on reflect of player, which
        sty REFP0,x     ; [ Store Y ] into REFP0; makes humanoid image face left 

CheckDown:
        asl                     ; [ Arithmetric Shift Left ] shift A bits left, D is now in the carry bit
        bcs CheckUp             ; [ Branch if Carry Set ] branch if joystick not held down
        ldy ObjectY,x           ; [ Load Y ] get the object's Y position
        dey                     ; [ Decrement Y ] move it down
        cpy #255                ; [ Compare Y ] to value 255; test for bottom of screen
        bne SaveY               ; [ Branch if Not Equal ] save Y if we're not at the bottom
        ldy #ARENA_HEIGHT*2+1   ; [ Load Y ] with value to wrap to top
SaveY:  sty ObjectY,x           ; [ Store Y ] save Y position

CheckUp:
        asl                     ; [ Arithmetric Shift Left ] shift A bits left, U is now in the carry bit
        bcs NextJoystick        ; [ Branch if Carry Set ] branch if joystick not held up
        ldy ObjectY,x           ; [ Load Y ] get the object's Y position
        iny                     ; [ Increment Y ] move it up
        cpy #ARENA_HEIGHT*2+2   ; [ Compare Y ] to top of screen; test for top of screen
        bne SaveY2              ; [ Branch if Not Equal ] save Y if we're not at the top
        ldy #0                  ; [ Load Y ] with value 0 to wrap to bottom
SaveY2: sty ObjectY,x           ; [ Store Y] save Y position
        
NextJoystick:     
        bit Players         ; [ test BIT ] test number of players by putting D7 into N
        bpl OnePlayer       ; [ Branch if Plus ] if N is off, it's a 1 player game so abort loop
        inx                 ; [ Increment X ] increase loop control
        cpx #2              ; [ Compare X ] to value 2; check if we've processed both joysticks
        bne PJloop          ; [ Branch not Equal ] branch if we haven't
        jmp Done

OnePlayer:  ; move the "second player" algorithmically       
        jsr MoveAIPlayer
...
; --------------
; 
; Each frame, randomly move 2nd player 1 pixel U/D/L/R or no move
;===============================================================================
MoveAIPlayer:
		ldx #1          ; set x for second player
        ldy ObjectX,x   ; [ Load Y ] save original X location so the player can be
        sty SavedX,x    ; [ Store Y ]   bounced back upon colliding with the playfield
        ldy ObjectY,x   ; [ Load Y ] save original Y location so the player can be
        sty SavedY,x    ; [ Store Y ]  bounced back upon colliding with the playfield

        jsr Random      ; [ Jump to Subroutine ] Random; get a random value between 0-255 into Accumulator
        and #7          ; [ AND Accumulator ] with value 127; limit range to 0-3
        sta Temp        ; [ Store Accumulator ] into memory location Temp; save it

CheckGoUp:
        cmp #0                  ; [ Compare accumulator ] with 0
        bne CheckGoLeft         ; [ Branch if Not Equal ] to check for next possibility
        ldy ObjectY,x           ; [ Load Y ] get the object's Y position
        iny                     ; [ Increment Y ] move it up
        cpy #ARENA_HEIGHT*2+2   ; [ Compare Y ] to top of screen; test for top of screen
        bne SaveY3              ; [ Branch if Not Equal ] save Y if we're not at the top
        ldy #0                  ; [ Load Y ] with value 0 to wrap to bottom
SaveY3: sty ObjectY,x           ; [ Store Y] save Y position
        rts
        
CheckGoLeft:
        cmp #1                  ; [ Compare accumulator ] with 1
        bne CheckGoDown         ; [ Branch if Not Equal ] to check for next possibility
        ldy ObjectX,x   ; [ Load Y ] get the object's X position
        dey             ; [ Decrement Y ] and move it left
        cpy #255        ; [ Compare Y ] to value 255; test for edge of screen
        bne SaveX3      ; [ Branch if Not Equal ] save X if we're not at the edge
        ldy #159        ; [ Load Y ] with value 159 to wrap to right edge
SaveX3: sty ObjectX,x   ; [ Store Y ] save X position
        ldy #8          ; [ Load Y] with value 8; turn on reflect of player, which
        sty REFP0,x     ; [ Store Y ] into REFP0; makes humanoid image face left 
        rts

CheckGoDown:
        cmp #2                  ; [ Compare accumulator ] with 2
        bne CheckGoRight        ; [ Branch if Not Equal ] to check for next possibility
        ldy ObjectY,x           ; [ Load Y ] get the object's Y position
        dey                     ; [ Decrement Y ] move it down
        cpy #255                ; [ Compare Y ] to value 255; test for bottom of screen
        bne SaveY4              ; [ Branch if Not Equal ] save Y if we're not at the bottom
        ldy #ARENA_HEIGHT*2+1   ; [ Load Y ] with value to wrap to top
SaveY4: sty ObjectY,x           ; [ Store Y ] save Y position
        rts

CheckGoRight: 
        cmp #3                  ; [ Compare accumulator ] with 3
        bne NoMove              ; [ Branch if Not Equal ] to check for next possibility
        ldy ObjectX,x   ; [ Load Y ] get the object's X position
        iny             ; [ Increment Y ] and move it right
        cpy #160        ; [ Compare Y ] to value 160; if Y = 160, flag Z will be set; test for edge of screen
        bne SaveX4      ; [ Branch Not Equal ] if flag Z not set, jump to SaveX; save Y if we're not at the edge
        ldy #0          ; [ Load Y ] with value 0 to wrap to left edge
SaveX4: sty ObjectX,x   ; [ Store Y ] save X position
        ldy #0          ; [ Load Y ] with value 0; turn off reflect of player, which
        sty REFP0,x     ; [ Store Y ] into REFP0; makes humanoid image face right
...
;===============================================================================
ProcessSwitches:
        lda SWCHB       ; [ Load Accumulator ] load in the state of the switches
        lsr             ; [ Logical Shift Right Accumulator ] D0 is now in Carry Flag
        bcs NotReset    ; [ Branch if Carry Set ] if D0 was on, the RESET switch was not held so jump past reset code
        ; Handle Reset pressed
        jsr NewGame     ; [ Jump to Subroutine ] NewGame; Prep for new game
        lda #%10000000  ; [ Load Accumulator ] with binary value 10000000 (D7 == 1, D0 through D6 == 0)
        sta GameState   ; [ Store Accumulator ] to memory location GameState; set D7 on to signify Game Active  
        bne NotSelect   ; [ Branch if accumulator Not Equal to zero ] to NotSelect; clear SelectDelay
        
NotReset:
        lsr             ; [ Logical Shift Right accumulator ] D1 is now in Carry Flag
        bcs NotSelect   ; [ Branch if Carry Set ] if D1 was on, the SELECT switch was not held so jump past select code
        ; Handle Select pressed - ignore it if it was previously pressed within the delay period specified by SelectDelay
        lda #0          ; [ Load Accumulator ] with value 0
        sta GameState   ; [ Store Accumulator ] into memory location GameState; clear D7 to signify Game Over Ê Ê Ê Ê
        lda SelectDelay ; [ Load Accumulator ] with memory location SelectDelay; do we need to delay the Select switch?
        beq SelectOK    ; [ Branch if accumulator Equal to zero ] if delay is 0 then no, jump to SelectOK
        dec SelectDelay ; [ Decrement ] the value at memory location SelectDelay; else decrement the delay
        rts             ; [ Return from Subroutine ] and exit the subroutine

SelectOK:
        lda #60         ; [ Load Accumulator ] with value 60; Set the Select Delay to 1 second (60 frames = 60 main loops = 1 sec)
        sta SelectDelay ; [ Store Accumulator ] into memory location SelectDelay
        ldx Variation   ; [ Load into X ] value at memory location Variation; Get the Game Variation
        inx             ; [ Increment X ] and increase it
        txa             ; [ Transfer X to A ] transfer it to A
        and #%00000011  ; [ AND Accumulator ] with binary value 00000011; limit Variation to 0-3 (binary 00000011)
        sta Variation   ; [ Store Accumulator ] into memory location Variation; save it
        tax             ; [ Transfer A to X ] transfer it to X
        inx             ; [ Increment X ] and increase it by 1 for the human readable varation 1-4
        stx Score       ; [ Store X ] into memory location Score; save in Score so it shows on left side
        ldy #1          ; [ Load Y ] with value 1; default to showing 1 player variation 
        lsr             ; [ Logical Shift Right accumulator ]; D0 of Variation (# of players) now in Carry flag
        bcc Not2        ; [ Branch if Carry Clear ] if Carry is clear, then show 1 player 
        iny             ; [ Incremnt Y ] else set Y to 2 to show 2 players
Not2:
        ror Players     ; [ Rotate Right accumulator] put Carry into D7 for BIT testing of # of players
        sty Score+1     ; [ Store Y ] into memory location Score + 1; show the human readable # of players on right side of score
        rts             ; [ Return from Subroutine ]
        
NotSelect: ; clear SelectDelay
        lda #0          ; [ Load Accumulator ] with value 0; clears SelectDelay if SELECT not held
        sta SelectDelay ; [ Store Accumulator ] into memory location SelectDelay
        rts             ; [ Return from Subroutine ]
...
NewGame:
    ; set starting location of player0 and player1 objects
        ...
        sta ObjectX     ; [ Store Accumulator ] into memory location for object X position
        sta REFP1       ; [ Store Accumulator ] bit D3 is on, so reflect player1
        lda #142        ; [ Load Accumulator ] with value 142
        sta ObjectX+1   ; [ Store Accumulator ] into memory location for object X position + 1
        lda #$63        ; [ Load Accumulator ] with value hex 63
        sta ObjectY     ; [ Store Accumulator ] into memory location for object Y position
        sta ObjectY+1   ; [ Store Accumulator ] into memory location for object Y position + 1
        sta REFP0       ; [ Store Accumulator ] bit D3 is off, so don't reflect player0
    ; reset timer to a solid bar (assymetric playfield = 6 bytes)       
        lda #%11111111  ; [ Load Accumulator ] with value binary 11111111
        sta TimerPF     ; [ Store Accumulator ] into memory location for Timer Playfield
        sta TimerPF+1   ; [ Store Accumulator ] into memory location for Timer Playfield + 1
        sta TimerPF+2   ; [ Store Accumulator ] into memory location for Timer Playfield + 2
        sta TimerPF+3   ; [ Store Accumulator ] into memory location for Timer Playfield + 3
        sta TimerPF+4   ; [ Store Accumulator ] into memory location for Timer Playfield + 4
        sta TimerPF+5   ; [ Store Accumulator ] into memory location for Timer Playfield + 5
    ...
    ; randomly placed in the Arena.
        lda Variation       ; [ Load Accumulator ] from memory location storing Game Variation
        and #1              ; [ AND Accumulator ] with 1; value of 0=1 player game, 1=2 player game
        tax                 ; [ Transfer A to X ] transfer to X
        inx                 ; [ Increment X ] start with 1 for a 1 player game, or 2 for a 2 player game
IPloop: ; For all 5 objects (Player 0, Player 1, Missile 0, Missile 1, Ball) set a random location
        jsr RandomLocation  ; [ Jump to Subroutine ] randomly position object specified by X
        inx                 ; [ Increment X] increase X for next object
        cpx #5              ; [ Compare X ] to 5; check if we hit 5
        bne IPloop          ; [ Branch on Not Equal ] branch back if we haven't
  
    ; reset scores
        ldx #0              ; [ Load X ] with value 0
        stx Score           ; [ Store X ] into memory location for Score
        bit Players         ; [ test Bits ] check # of players (bit D7: 0 = 1 player, 1 = 2 players)
                            ; About the BIT instruction:
                            ...
                            ; See https://dwheeler.com/6502/oneelkruns/asm1step.html
        bpl BlankRightScore ; [ Branch on Plus ] branch to Blank Right Score if flag N==0 (Players == 0 so 1 Player game)
        stx Score+1         ; [ Store X ] into memory locaiton for Score+1 (Player 2)
        rts                 ; [ Return from Subroutine ]
        
BlankRightScore:
        lda #$AA            ; [ Load Accumulator ] with hex value AA; AA defines a "space" character
        sta Score+1         ; [ Store Accumulator ] into Score for Player 2
        rts                 ; [ Return from Subroutine ]
...
; the Arena border we need to shift that 4 to the right so the X position
...
;===============================================================================
RandomLocation:
        ; Generate X within bounds
        jsr Random      ; [ Jump to Subroutine ] Random; get a random value between 0-255 into Accumulator
        and #127        ; [ AND Accumulator ] with value 127; limit range to 0-127
        sta Temp        ; [ Store Accumulator ] into memory location Temp; save it
        jsr Random      ; [ Jump to Subroutine ] Random; get a random value between 0-255
        and #15         ; [ AND Accumulator ] with value 15; limit range to 0-15
        clc             ; [ Clear Carry flag ] must clear carry for add
        adc Temp        ; [ Add to Accumulator with Carry ] add in random # from 0-127 for range of 0-142
        adc #5          ; [ Add to Accumulator with Carry ] add 5 for range of 5-147
        sta ObjectX,x   ; [ Store Accumulator ] into memory location ObjectX + X; save the random X position for object indexed by X
 
        ; Generate Y within bounds        
        jsr Random      ; [ Jump to Subroutine ] Random; get a random value between 0-255 into Accumulator
        and #127        ; [ AND Accumulator ] with value 127; limit range to 0-127
        sta Temp        ; [ Store Accumulator ] into memory location Temp; save it
        jsr Random      ; [ Jump to Subroutine ] Random; get a random value between 0-255
        ...
        clc             ; [ Clear Carry flag ] must clear carry for add
        adc Temp        ; [ Add to Accumulator with Carry ] add in random # from 0-127 for range of 0-142
        adc #26         ; [ Add to Accumulator with Carry ] add 26 for range of 26-168
        sta ObjectY,x   ; [ Store Accumulator ] into memory location ObjectY + X; save the random Y position for object indexed by X
    ...
    ;   sta ObjectY,x
        ...
        ; Missile and ball objects need X position incremented by 1 
        cpx #2          ; [ Compare X ] with value 2; Carry will be set if X >= 2 (if object is a missile or ball)
        bcc RLdone      ; [ Branch if Carry flag set ] to RLdone (meaning this object is a player) 
        inc ObjectX,x   ; [ Increment ] memory location at ObjectX + X; missile and ball objects need their X adjusted
RLdone:        
        rts             ; [ Return from Subroutine ]
...
;=============================================================================== 
Random:
        lda Rand8       ; [ Load Accumulator ] from memory location Rand8
        lsr             ; [ Logical Shift Right the accumulator ]
 ifconst Rand16
        rol Rand16      ; [ Rotate Left ] the contents of memory location Rand16
                        ; this command is only used if Rand16 has been defined
 endif
        bcc noeor       ; [ Branch on Carry Clear ] to "noeor" if Carry==0
                        ; the LSR above shifted bit 0 of the accumulator to the Carry flag
        eor #$B4        ; [ XOR Accumulator ] with hex value B4 per the Galois LSFR algorithm
noeor 
        sta Rand8       ; [ Store Accumulator ] into memory location Rand8
 ifconst Rand16
        eor Rand16      ; [ XOR Accumulator ] with memory location Rand16 
                        ; this command is only used if Rand16 has been defined
 endif
        rts             ; [ Return from Subroutine ]
